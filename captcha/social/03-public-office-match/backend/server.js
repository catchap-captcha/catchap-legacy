/**
 * CatChap · 사회 캡챠 — Express 서버 (공공기관 역할 연결)
 * ---------------------------------------------------------------
 *   npm install
 *   npm start        (또는 npm run dev)
 *   → http://localhost:4803  에서 데모 페이지가 열립니다.
 */
...
// API 라우트
app.use('/api/public-office', captchaRouter);

(async function boot() {
  try {
    await initSchema();
    console.log('✅ DB 스키마 준비 완료 (catchap_social · office_*)');
  } catch (err) {
  ...
  app.listen(PORT, () => {
    console.log(`\n🐱 CatChap 사회 CAPTCHA · 공공기관 역할 연결`);
    console.log(`   데모:  http://localhost:${PORT}/`);
    console.log(`   API :  http://localhost:${PORT}/api/public-office/health\n`);
  });
})();
