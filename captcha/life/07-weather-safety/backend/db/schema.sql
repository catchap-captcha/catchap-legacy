...
--  같은 catchap_life DB 공유, 프리픽스 weather_ 로 분리.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS weather_session (
  session_id     CHAR(36)      NOT NULL,
  captcha_type   VARCHAR(32)   NOT NULL DEFAULT 'weather-safety',
  ...
  INDEX idx_weather_session_status (status),
  INDEX idx_weather_session_started (started_at)
) ENGINE=InnoDB;

-- 통합 시도 로그 : single/connect/pick/sort/touch/place/route 전부 수용
CREATE TABLE IF NOT EXISTS weather_attempt (
  ...
  INDEX idx_weather_attempt_session (session_id),
  INDEX idx_weather_attempt_question (question_id),
  INDEX idx_weather_attempt_stage (stage),
  CONSTRAINT fk_weather_attempt_session
    FOREIGN KEY (session_id) REFERENCES weather_session(session_id)
    ON DELETE CASCADE
) ENGINE=InnoDB;
