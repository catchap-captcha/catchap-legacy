/**
 * CatChap · 사회 캡챠 — Express 서버 (__TITLE__)
 * ---------------------------------------------------------------
 *   npm install
 *   npm start        (또는 npm run dev)
 *   → http://localhost:__PORT__  에서 데모 페이지가 열립니다.
 */
...
// API 라우트
app.use('/api/__API_PATH__', captchaRouter);

(async function boot() {
  try {
    await initSchema();
    console.log('✅ DB 스키마 준비 완료 (catchap_social · __PREFIX___*)');
  } catch (err) {
  ...
  app.listen(PORT, () => {
    console.log(`\n🐱 CatChap 사회 CAPTCHA · __TITLE__`);
    console.log(`   데모:  http://localhost:${PORT}/`);
    console.log(`   API :  http://localhost:${PORT}/api/__API_PATH__/health\n`);
  });
})();
