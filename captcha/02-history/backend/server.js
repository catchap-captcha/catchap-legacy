/**
 * CatChap · 우리 지역 문화유산 CAPTCHA — Express 서버
 * ---------------------------------------------------------------
 *   npm install
 *   npm start        (또는 npm run dev)
 *   → http://localhost:5520  에서 데모 페이지가 열립니다.
 */
...
// API 라우트
app.use('/api/history', captchaRouter);

(async function boot() {
  try {
    await initSchema();
    console.log('✅ DB 스키마 준비 완료 (catchap_quiz · history_*)');
  } catch (err) {
  ...
  app.listen(PORT, () => {
    console.log(`\n🐱 CatChap 우리 지역 문화유산 CAPTCHA`);
    console.log(`   데모:  http://localhost:${PORT}/`);
    console.log(`   API :  http://localhost:${PORT}/api/history/health\n`);
  });
})();
