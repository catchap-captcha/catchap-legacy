/**
 * Grammar Choice CAPTCHA — API 라우트
 *   POST /api/grammar-choice/start | /attempt | /verify
 *   GET  /api/grammar-choice/token/:token | /health
 */
...
router.post('/start', async (req, res) => {
  try {
    ...
    await tryQuery(
      `INSERT INTO grammar_session (session_id, captcha_type, status, ip_address, user_agent)
       VALUES (:sid, 'grammar-choice', 'in_progress', :ip, :ua)`, { sid: sessionId, ip, ua });
...
router.post('/attempt', async (req, res) => {
  try {
    const { sessionId, questionId, selectedAnswer, metrics = {} } = req.body || {};
    ...
    const ok = String(selectedAnswer || '') === q.answer ? 1 : 0;

    await tryQuery(
      `INSERT INTO grammar_attempt (
         session_id, question_id, stage, is_correct,
         grammar_type, sentence_masked, target_answer, selected_answer,
         wrong_attempt_count, hovered_options_json, first_select_time_ms, hesitation_time_ms,
         solve_time_ms, retry_count, metrics_json
       ) VALUES (
         :session_id, :question_id, :stage, :is_correct,
         :grammar_type, :sentence_masked, :target_answer, :selected_answer,
         :wrong_attempt_count, :hovered_options_json, :first_select_time_ms, :hesitation_time_ms,
       ...
       )`,
      {
        ...
        grammar_type: q.grammarType, sentence_masked: q.sentence,
        target_answer: q.answer, selected_answer: selectedAnswer ?? null,
    ...
    await tryQuery(
      `UPDATE grammar_session SET total_answered = total_answered + 1, total_correct = total_correct + :inc
  ...
  try {
    ...
    const dbRes = await tryQuery(
      `SELECT stage, SUM(is_correct) AS correct, COUNT(*) AS answered
         FROM grammar_attempt WHERE session_id = :sid GROUP BY stage`, { sid: sessionId });
    ...
    await tryQuery(
      `UPDATE grammar_session SET status = :status, pass_token = :token, total_correct = :tc, finished_at = NOW()
...
router.get('/token/:token', async (req, res) => {
  const dbRes = await tryQuery(`SELECT session_id FROM grammar_session WHERE pass_token = :t AND status='passed' LIMIT 1`, { t: req.params.token });
