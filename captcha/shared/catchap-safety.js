...
(function (global) {
  ...
  function createInstance(container, opts) {
    ...
    function buildSingle(card, q, qs) {
      const list = el('div', 'csl-options');
      q.options.forEach((opt) => {
        const btn = el('button', 'csl-option');
        btn.dataset.id = opt.id;
        if (opt.emoji) btn.appendChild(el('span', 'csl-option-emoji', opt.emoji));
        btn.appendChild(el('span', 'csl-option-text', opt.text));
        btn.onclick = () => {
          markFirst(qs);
    ...
    // ── connect : 왼쪽 항목을 누르고 오른쪽 항목을 눌러 짝을 짓는다 ──
    function buildConnect(card, q, qs) {
      const palette = ['#ff5a8a', '#ff9a3d', '#34c759', '#4a9dff', '#b06bff'];
      ...
      q.left.forEach((it) => {
        const b = el('button', 'csl-conn-item');
        b.dataset.id = it.id;
        b.appendChild(el('span', 'csl-conn-badge', ''));
        b.appendChild(el('span', 'csl-conn-text', it.text));
        leftEls[it.id] = b;
        leftCol.appendChild(b);
      });
      shuffle(q.right).forEach((it) => {
        const b = el('button', 'csl-conn-item csl-conn-right');
        b.dataset.id = it.id;
        b.appendChild(el('span', 'csl-conn-text', it.text));
        b.appendChild(el('span', 'csl-conn-badge', ''));
        rightEls[it.id] = b;
      ...
      function paintPair(leftId, rightId) {
        const c = colorOf(leftId), lb = leftEls[leftId], rb = rightEls[rightId];
        const badge = q.left.findIndex((x) => x.id === leftId) + 1;
        lb.style.borderColor = c; rb.style.borderColor = c;
        const lbg = lb.querySelector('.csl-conn-badge'), rbg = rb.querySelector('.csl-conn-badge');
        lbg.textContent = badge; lbg.style.background = c;
        rbg.textContent = badge; rbg.style.background = c;
      }
      function clearRight(rightId) {
        const rb = rightEls[rightId]; rb.style.borderColor = '';
        const rbg = rb.querySelector('.csl-conn-badge'); rbg.textContent = ''; rbg.style.background = '';
      }
      function clearLeft(leftId) {
        const lb = leftEls[leftId]; lb.style.borderColor = '';
        const lbg = lb.querySelector('.csl-conn-badge'); lbg.textContent = ''; lbg.style.background = '';
      }
      Object.values(leftEls).forEach((b) => {
        b.onclick = () => {
          markFirst(qs);
          Object.values(leftEls).forEach((x) => x.classList.remove('csl-active'));
          activeLeft = b.dataset.id; b.classList.add('csl-active');
        };
      });
      Object.values(rightEls).forEach((b) => {
        b.onclick = () => {
          ...
          markFirst(qs);
          const rightId = b.dataset.id;
          for (const l of Object.keys(qs.pairs)) {
            if (qs.pairs[l] === rightId) { delete qs.pairs[l]; clearLeft(l); qs.reconnectCount += 1; }
          }
          if (qs.pairs[activeLeft]) { clearRight(qs.pairs[activeLeft]); qs.reconnectCount += 1; }
          qs.pairs[activeLeft] = rightId;
          paintPair(activeLeft, rightId);
          leftEls[activeLeft].classList.remove('csl-active');
          activeLeft = null;
          qs.refreshSubmit();
        };
      });
    ...
    async function onSubmit(q, qs) {
      ...
      if (result.correct) {
        ...
        qs.feedbackEl.textContent = q.successText || '잘했어요! 안전을 잘 지켰어요 🎉';
        qs.feedbackEl.className = 'csl-feedback csl-ok';
      } else {
        qs.feedbackEl.textContent = q.failText || '아쉬워요, 다음에 더 안전하게! 💪';
        qs.feedbackEl.className = 'csl-feedback csl-no';
      }
