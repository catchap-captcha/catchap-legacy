-- ============================================================================
--  CatChap · 미아 안전 캡챠  —  Database Schema (MySQL 8+)
-- ----------------------------------------------------------------------------
--  06~10 은 조작 유형이 다양(터치/연결/좌표드래그/분류/경로)해서
--  모든 유형을 담는 통합(superset) 시도 테이블을 사용한다.
--  같은 catchap_life DB 공유, 프리픽스 lost_ 로 분리.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS lost_session (
  session_id     CHAR(36)      NOT NULL,
  captcha_type   VARCHAR(32)   NOT NULL DEFAULT 'lost-child',
  ...
  INDEX idx_lost_session_status (status),
  INDEX idx_lost_session_started (started_at)
) ENGINE=InnoDB;

-- 통합 시도 로그 : single/connect/pick/sort/touch/place/route 전부 수용
CREATE TABLE IF NOT EXISTS lost_attempt (
  ...
  INDEX idx_lost_attempt_session (session_id),
  INDEX idx_lost_attempt_question (question_id),
  INDEX idx_lost_attempt_stage (stage),
  CONSTRAINT fk_lost_attempt_session
    FOREIGN KEY (session_id) REFERENCES lost_session(session_id)
    ON DELETE CASCADE
) ENGINE=InnoDB;
