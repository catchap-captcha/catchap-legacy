...
--  같은 catchap_quiz DB 공유, 프리픽스 history_ 로 분리.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS history_session (
  ...
  captcha_type   VARCHAR(32)   NOT NULL DEFAULT 'history',
  ...
  INDEX idx_history_session_status (status),
  INDEX idx_history_session_started (started_at)
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS history_attempt (
  ...
  INDEX idx_history_attempt_session (session_id),
  INDEX idx_history_attempt_question (question_id),
  INDEX idx_history_attempt_stage (stage),
  CONSTRAINT fk_history_attempt_session
