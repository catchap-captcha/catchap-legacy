/**
 * CatChap · 사회 캡챠 — Express 서버 (지역 축제 포스터 찾기)
 * ---------------------------------------------------------------
 *   npm install
 *   npm start        (또는 npm run dev)
 *   → http://localhost:4807  에서 데모 페이지가 열립니다.
 */
...
// API 라우트
app.use('/api/festival', captchaRouter);

(async function boot() {
  try {
    await initSchema();
    console.log('✅ DB 스키마 준비 완료 (catchap_social · festival_*)');
  } catch (err) {
  ...
  app.listen(PORT, () => {
    console.log(`\n🐱 CatChap 사회 CAPTCHA · 지역 축제 포스터 찾기`);
    console.log(`   데모:  http://localhost:${PORT}/`);
    console.log(`   API :  http://localhost:${PORT}/api/festival/health\n`);
  });
})();
