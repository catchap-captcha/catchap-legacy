/**
 * 사회(社會) 캡챠 — 통합 API 라우트 (모든 조작 유형 지원)
 * ---------------------------------------------------------------
 ...
 *  GET   /health         헬스체크
 *
 *  지원 유형: single / order / connect / pick / sort / touch / place / route
 *  ⚙️ 캡챠별로 다른 값은 아래 CONFIG 뿐이다. (10종 공통 엔진)
 */
...
// ── 캡챠별 설정 (여기만 캡챠마다 다름) ──
const CONFIG = {
  captchaType: 'COMMUNITY_PROBLEM_SOLVE',
  tableSession: 'community_session',
  tableAttempt: 'community_attempt',
};
...
function judge(question, payload) {
  switch (question.type) {
    ...
    case 'order': {
      ...
      let wrong = 0;
      for (let i = 0; i < target.length; i++) if (seq[i] !== target[i]) wrong += 1;
      return { correct: wrong === 0 && seq.length === target.length, wrongOrderCount: wrong, target };
    }
    case 'connect': {
...
function publicVerdict(v) {
  const out = {};
  ['wrongOrderCount', 'correctConnectionCount', 'wrongConnectionCount', 'wrongSelectedCount',
   'missedCount', 'wrongSortCount', 'reachedDest', 'dangerTouchedCount'].forEach((k) => { if (v[k] != null) out[k] = v[k]; });
  return out;
}
  ...
  try {
    ...
    await tryQuery(
      `INSERT INTO ${CONFIG.tableAttempt} (
         ...
         selected_sequence_json, target_sequence_json, wrong_order_count,
