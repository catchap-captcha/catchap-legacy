-- ============================================================================
--  CatChap · 놀이터 안전 캡챠  —  Database Schema (MySQL 8+)
-- ----------------------------------------------------------------------------
--  06~10 은 조작 유형이 다양(터치/연결/좌표드래그/분류/경로)해서
--  모든 유형을 담는 통합(superset) 시도 테이블을 사용한다.
--  같은 catchap_life DB 공유, 프리픽스 play_ 로 분리.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS play_session (
  ...
  captcha_type   VARCHAR(32)   NOT NULL DEFAULT 'playground-safety',
  ...
  INDEX idx_play_session_status (status),
  INDEX idx_play_session_started (started_at)
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS play_attempt (
  ...
  INDEX idx_play_attempt_session (session_id),
  INDEX idx_play_attempt_question (question_id),
  INDEX idx_play_attempt_stage (stage),
  CONSTRAINT fk_play_attempt_session
