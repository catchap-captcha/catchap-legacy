...
// ── 캡챠별 설정 (여기만 캡챠마다 다름) ──
const CONFIG = {
  captchaType: 'geography',
  tableSession: 'geo_session',
  tableAttempt: 'geo_attempt',
};

const router = express.Router();
const uuid = () => crypto.randomUUID();

// 45문제(단계별 9개) 중 단계마다 5개씩 무작위로 뽑아 25문제를 만든다(난이도 오름차순).
const QUESTIONS_PER_STAGE = 5;
const shuffleQ = (a) => { const x = a.slice(); for (let i = x.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [x[i], x[j]] = [x[j], x[i]]; } return x; };
const pickQuestions = () => { const out = []; for (let s = 1; s <= 5; s++) out.push(...shuffleQ(QUESTIONS.filter((q) => q.stage === s)).slice(0, QUESTIONS_PER_STAGE)); return out; };
...
router.post('/start', async (req, res) => {
  try {
    ...
    res.json({
      ...
      questions: pickQuestions().map(sanitizeQuestion),
    });
  } catch (err) {
