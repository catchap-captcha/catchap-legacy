/**
 * CatChap · 사회 캡챠 — Express 서버 (심폐소생술·자동심장충격기 안전 순서)
 * ---------------------------------------------------------------
 *   npm install
 *   npm start        (또는 npm run dev)
 *   → http://localhost:4810  에서 데모 페이지가 열립니다.
 */
...
// API 라우트
app.use('/api/cpr-aed', captchaRouter);

(async function boot() {
  try {
    await initSchema();
    console.log('✅ DB 스키마 준비 완료 (catchap_social · cpr_*)');
  } catch (err) {
  ...
  app.listen(PORT, () => {
    console.log(`\n🐱 CatChap 사회 CAPTCHA · 심폐소생술·자동심장충격기 안전 순서`);
    console.log(`   데모:  http://localhost:${PORT}/`);
    console.log(`   API :  http://localhost:${PORT}/api/cpr-aed/health\n`);
  });
})();
