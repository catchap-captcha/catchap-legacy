-- ============================================================================
--  CatChap · 화재안전 캡챠  —  Database Schema (MySQL 8+)
-- ----------------------------------------------------------------------------
--  실행 방법:
...
-- ────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS fire_session (
  session_id     CHAR(36)      NOT NULL,
  captcha_type   VARCHAR(32)   NOT NULL DEFAULT 'fire-safety',
  ...
  INDEX idx_fire_session_status (status),
  INDEX idx_fire_session_started (started_at)
) ENGINE=InnoDB;

-- ────────────────────────────────────────────────────────────────
--  2) 문제별 시도(행동) 로그
--     유형(single/order/connect)별 컬럼 + 자유형 metrics_json
-- ────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS fire_attempt (
  ...
  selected_pairs_json    JSON  NULL,   -- 화재: 행동-이유 연결 결과
  ...
  INDEX idx_fire_attempt_session (session_id),
  INDEX idx_fire_attempt_question (question_id),
  INDEX idx_fire_attempt_stage (stage),
  CONSTRAINT fk_fire_attempt_session
    FOREIGN KEY (session_id) REFERENCES fire_session(session_id)
    ON DELETE CASCADE
) ENGINE=InnoDB;
