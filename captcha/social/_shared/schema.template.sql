...
-- ────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS __PREFIX___session (
  session_id     CHAR(36)      NOT NULL,
  captcha_type   VARCHAR(48)   NOT NULL DEFAULT '__CAPTCHA_TYPE__',
  ...
  INDEX idx___PREFIX___session_status (status),
  INDEX idx___PREFIX___session_started (started_at)
) ENGINE=InnoDB;

-- ────────────────────────────────────────────────────────────────
--  2) 문제별 시도(행동) 로그 : single/order/connect/pick/sort/touch/place/route 전부 수용
-- ────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS __PREFIX___attempt (
  ...
  INDEX idx___PREFIX___attempt_session (session_id),
  INDEX idx___PREFIX___attempt_question (question_id),
  INDEX idx___PREFIX___attempt_stage (stage),
  CONSTRAINT fk___PREFIX___attempt_session
    FOREIGN KEY (session_id) REFERENCES __PREFIX___session(session_id)
    ON DELETE CASCADE
) ENGINE=InnoDB;
