-- ============================================================================
--  CatChap · 긴급 전화번호 캡챠 (드래그형)  —  Database Schema (MySQL 8+)
-- ----------------------------------------------------------------------------
--  실행: mysql -u root -p < schema.sql  (또는 서버 기동 시 자동 생성)
--  같은 catchap_life DB 공유, 테이블 프리픽스 emergency_ 로 분리.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS emergency_session (
  session_id     CHAR(36)      NOT NULL,
  captcha_type   VARCHAR(32)   NOT NULL DEFAULT 'emergency-number',
  ...
  INDEX idx_emergency_session_status (status),
  INDEX idx_emergency_session_started (started_at)
) ENGINE=InnoDB;
...
-- ────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS emergency_attempt (
  ...
  INDEX idx_emergency_attempt_session (session_id),
  INDEX idx_emergency_attempt_question (question_id),
  INDEX idx_emergency_attempt_stage (stage),
  CONSTRAINT fk_emergency_attempt_session
    FOREIGN KEY (session_id) REFERENCES emergency_session(session_id)
    ON DELETE CASCADE
) ENGINE=InnoDB;
