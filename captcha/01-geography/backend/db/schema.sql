...
--  같은 catchap_quiz DB 공유, 프리픽스 geo_ 로 분리.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS geo_session (
  session_id     CHAR(36)      NOT NULL,
  captcha_type   VARCHAR(32)   NOT NULL DEFAULT 'geography',
  ...
  INDEX idx_geo_session_status (status),
  INDEX idx_geo_session_started (started_at)
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS geo_attempt (
  ...
  INDEX idx_geo_attempt_session (session_id),
  INDEX idx_geo_attempt_question (question_id),
  INDEX idx_geo_attempt_stage (stage),
  CONSTRAINT fk_geo_attempt_session
    FOREIGN KEY (session_id) REFERENCES geo_session(session_id)
    ON DELETE CASCADE
) ENGINE=InnoDB;
