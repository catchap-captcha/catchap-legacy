/**
 * CatChap · 손씻기 CAPTCHA — Express 서버
 * ---------------------------------------------------------------
 *   npm install
 *   npm start        (또는 npm run dev)
 *   → http://localhost:4900  에서 데모 페이지가 열립니다.
 */
...
// API 라우트
app.use('/api/clean-hands', captchaRouter);

(async function boot() {
  try {
    await initSchema();
    console.log('✅ DB 스키마 준비 완료 (catchap_life · hands_*)');
  } catch (err) {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`   API :  http://localhost:${PORT}/api/clean-hands/health\n`);
  });
})();
